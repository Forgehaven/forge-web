...
const STATION_LABELS: Record<string, string> = Object.fromEntries(
  STATION_TYPES.map(s => [s.value, s.label]),
)
...
// Coarse item category for the universal Item Index, derived from the id (price-free). Reuses
// itemEcon's archetype classification so the label lines up with the Station column.
export function itemCategory(id: string): string {
  const base = id.split('@')[0]
  if (isResource(base)) return 'Resource'
  const econ = itemEcon(id)
  if (!econ) return 'Other'
  if (/(HEAD|ARMOR|SHOES)_(CLOTH|LEATHER|PLATE)/.test(base)) return 'Armour'
  if (econ.station === 'toolmaker') {
    if (/BAG/.test(base)) return 'Bag'
    if (/CAPE/.test(base)) return 'Cape'
    return 'Tool'
  }
  if (econ.station === 'cook') return 'Food'
  if (econ.station === 'alchemists_lab') return 'Potion'
  if (/(^|_)OFF_/.test(base)) return 'Off-hand'
  if (econ.station === 'forge') return 'Warrior Weapon'
  if (econ.station === 'hunters_lodge') return 'Hunter Weapon'
  if (econ.station === 'mages_tower') return 'Mage Weapon'
  return 'Other'
}
