...
import type { AlbionItem, RecipeNode } from './types'

// Price-free data access for the universal crafting tools: the full item list and recipes
// (bill of materials). None of these return live market prices - user prices live in the
// per-user store (see userStore).
...
export async function fetchAllItems(): Promise<Envelope<AlbionItem[]>> {
  return forgeFetch<AlbionItem[]>('/game/albion/items?limit=0')
}
